...
cv::Mat Calibrator::processFrame(BGRAVideoFrame frame) {
    ...
    {
        case CHESSBOARD: {
            int cornerFlag = CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_NORMALIZE_IMAGE | CALIB_CB_FILTER_QUADS;
//            int cornerFlag = 0;
    ...
    if( pattern == CHESSBOARD && found) cornerSubPix( viewGray, pointbuf, cv::Size(20,20),
                                                     cv::Size(-1,-1), TermCriteria( TermCriteria::Type::EPS+TermCriteria::Type::MAX_ITER, 30, 0.1 ));
...
void Calibrator::calcChessboardCorners(cv::Size boardSize, float squareSize, vector<Point3f>& corners, Pattern patternType)
{
    ...
    switch(patternType)
    {
        ...
        default:
            CV_Error(cv::Error::Code::StsBadArg, "Unknown pattern type\n");
    }
}
                                  ...
                                  double totalAvgErr )
{
    ...
    if( flags != 0 )
    {
        sprintf( buf, "flags: %s%s%s%s",
                ...
                CALIB_FIX_PRINCIPAL_POINT ? "+fix_principal_point" : "",
                CALIB_ZERO_TANGENT_DIST ? "+zero_tangent_dist" : "" );
        write(fs, buf, 0);
    }
    ...
    if( !rvecs.empty() && !tvecs.empty() )
    {
        ...
        write( fs, "a set of 6-tuples (rotation vector + translation vector) for each view", 0 );
